...
def main_clust_exp(args, argparser):
    ...
    for design in search_params_dict['designs']:
        ...
        for method in search_params_dict['methods']:
            dict_diff[method] = uo.read_diff_table(
                args, file_dict[method], method, dir_design,
                args.PVALUE, list_genes
            )

            # for method in search_params_dict['methods']:
            for top in search_params_dict['tops']:
                for num_c in search_params_dict['num_cluster']:
                    print("##{}, {}, TOP {}, CLUSETER {}".format(
                            design, method.upper(), str(top), str(num_c)
                        )
                    )
                    silh_score = uo.get_exp_cluster(
                        args, df_exp, df_design,
                        dict_diff[method], path_out, method, design,
                        top, num_c, max_exp
                    )
